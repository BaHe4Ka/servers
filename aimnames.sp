...
{
	name = "Aim Names",
	author = "-MCG-Retsam & Antithasys | Redacted by di",
	description = "Shows enemy names when crosshair is placed on them",
...
public OnPluginStart()
{
	...
	g_Cvar_Color = CreateConVar("sm_aimnames_color", "2", "Default color for hudtext. (0 = red, 1 = orange, 2 = yellow, 3 = green, 4 = blue)");
	g_Cvar_Admins = CreateConVar("sm_aimnames_adminonly", "0", "Enable/Disable plugin for admins only.");
	g_Cvar_Flag = CreateConVar("sm_aimnames_flag", "a", "Admin flag(s) to use if admin only.");
	g_Cvar_Health = CreateConVar("sm_aimnames_health", "1", "Enable/Disable admins viewing players health.");
  g_Cvar_Holdtime = CreateConVar("sm_aimnames_holdtime", "0.1", "(Default: 0.4) The hudtext number of seconds to hold/refresh the text.");
  g_Cvar_Distance = CreateConVar("sm_aimnames_distance", "100000.0", "(Default: 100) Distance in meters for showing aim names.");
  g_Cvar_TimerInterval = CreateConVar("sm_aimnames_timerinterval", "0.2", "(Default: 0.3) Repeat timer interval. Time in seconds between execing the timer function.");
...
public Action:Timer_SyncHud(Handle:timer, any:client)
{
    ...
    new iTarget = GetClientAimTarget(client, true);
		...
		if (CanSeeTarget(client, client_pos, iTarget, target_pos, g_fDistance))
		{
			...
			{
				SetHudTextParams(-1.0, 0.44, g_fHoldtime, g_iColors[e_ColorNames:g_aPlayers[client][eColor]][iRed], g_iColors[e_ColorNames:g_aPlayers[client][eColor]][iGreen], g_iColors[e_ColorNames:g_aPlayers[client][eColor]][iBlue], 255, 1);
